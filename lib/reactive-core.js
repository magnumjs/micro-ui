...
function restoreActiveElement() {
  //add query to the activeElement in the compoennt
  if (!this.el.contains(activeElement) && activeElement?.id) {
    const foundEle = this.el?.querySelector(
      `#${activeElement.id}, [name='${activeElement.name}'], ${activeElement.tagName}`
    );
    if (foundEle) {
      activeElement = foundEle;
    }
  }
  ...
  if (activeElement && document.contains(activeElement)) {
    try {
      ...
      if (
        ...
        activeElement.setSelectionRange(activeElement.selectionStart, activeElement.selectionEnd);
      }
    } catch {
      // ignore if not supported or if element lost focus
    }
  }
}
...
document.addEventListener("focusout", (e) => {
  if (e.target === activeElement) activeElement = null;
});
...
export function createComponent(arg1, arg2 = {}) {
  ...
  api.onUpdate(restoreActiveElement)
  ...
  function render(currentProps) {
    ...
    if(html && html.nodeType === 1) {
      // keep data-key updated on root element
    //  if (!html.hasAttribute("data-key")) html.setAttribute("data-key", componentFn._id);
    }
