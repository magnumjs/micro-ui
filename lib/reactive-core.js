...
import { cacheNodes, restoreNodes } from './core/cacheNodes.js';
...
import { shallowEqual } from './core/propsHelpers.js';
...
export function createComponent(arg1, arg2 = {}) {
  ...
  const setState = (next) => {
    ...
    //define prevState
    // shallow compare state to prevState
  ...
  const api = {
    ...
    mount(targetOrSelector, initialProps = {}) {
      ...
      const proceed = () => {
        ...
        // register in local key registry if applicable
        if (props.key) {
          localKeyRegistry.set(props.key, componentFn);
        }
    ...
    unmount() {
      ...
      const cleanup = () => {
        ...
        // cleanup key registry if applicable
        if (props.key && localKeyRegistry.has(props.key)) {
          localKeyRegistry.delete(props.key);
        }
      };
  ...
  function render(currentProps) {
    ...
    if (mounted && (html === null || html === '')) {
      if (!_renderedNull && el && el.hasChildNodes()) {
        _cachedNode = cacheNodes(el);
        runBeforeHook(
          api._onBeforeUnmountCbs,
          () => {
            boundEvents = cleanupDomAndEvents(el, boundEvents);
            mounted = false;
            _renderedNull = true;
            runHook(api._onUnmountCbs, undefined, true, componentFn);
          },
          api,
        );
      } else {
    ...
    if (mounted) {
      runHook(api._onUpdateCbs, prevProps, false, componentFn);
    }
  ...
  function componentFn(props = {}) {
    ...
    if (
      ...
      // console.log('Child component detected:', componentFn._id);
    ...
    if (
      ...
      // console.log(
      //   'Creating new keyed clone for:',
      //   props.key
      // );
      // Only the original factory participates in the keyed registry logic
      ...
      // If this is a mount call, ensure the instance is rendered/mounted
      // if (componentFn.getCurrentLifecycleState() === 'mounting') {
      //   currentComponent = componentFn;
      //   lastHtml = componentFn._render(props); // or pinst.render(props) if appropriate
  ...
  componentFn.clone = function (initialProps = {}) {
    ...
    return __cloneFromTemplate(componentFn, initialProps);
  };
