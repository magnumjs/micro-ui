function restoreActiveElement(active) {
  if (active && document.contains(active)) {
    try {
      ...
      if (
        ...
        active.setSelectionRange(active.selectionStart, active.selectionEnd);
      }
    } catch {
...
export function createComponent(arg1, arg2 = {}) {
  ...
  function render(currentProps) {
    ...
    if (_renderedNull && _cachedNode) {
      ...
      // set active element before diff to the component root
     if(document.activeElement)   componentFn._active = document.activeElement;
    ...
    // reset active element after diff
    restoreActiveElement(componentFn._active);
