...
export function createComponent(arg1, arg2 = {}) {
  ...
  const api = {
    ...
    mount(targetOrSelector, initialProps = {}) {
      ...
      const proceed = () => {
        ...
        el.setAttribute("data-key", props.key || componentFn._id);
  ...
  function render(currentProps) {
    ...
    if (_renderedNull && _cachedNode) {
      ...
      // Ensure the cached node has the correct data-key
      el.firstChild?.setAttribute("data-key", props.key || componentFn._id);
      _cachedNode = null;
      _renderedNull = false;
      mounted = true;
