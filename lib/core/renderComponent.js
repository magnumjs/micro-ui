...
import { syncInstanceToAPI } from "./syncInstanceToAPI.js";
...
export function renderComponent({ renderFn, componentFn, api, state, setState, props, newProps }) {
  ...
  // Short-circuit: skip DOM update if HTML is unchanged
  if (api._lastHtml !== undefined && String(html) === String(api._lastHtml)) {
    return api._lastHtml;
  }
  api._lastHtml = html;
